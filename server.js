...
async function shutdownService(name, action) {
  try { await action(); } catch (error) { console.error(`${name} shutdown failed.`); console.error(error.message); if (process.env.APP_ENV !== 'production') console.error(error.stack); }
}
...
function registerSignalHandlers({ waClient, telegramBot }) {
  async function handleShutdown(signal) {
    ...
    await shutdownService('Puppeteer browser', async () => { await closeBrowser(); });
    process.exit(0);
  }
...
async function startServer() {
  let waClient; let telegramBot;

  if (CONFIG.WHATSAPP.ENABLED) {
    const deletedLocks = cleanupWhatsAppAuthLocks();
    if (deletedLocks.length > 0) console.log(`[startup] Deleted ${deletedLocks.length} WhatsApp auth lock file(s).`);
    try { const { createBot } = require('./src/bot'); waClient = await createBot(); } catch (error) { console.error('Failed to start WhatsApp bot.'); console.error(error.message); if (process.env.APP_ENV !== 'production') console.error(error.stack); throw error; }
  } else {
    console.log('WhatsApp bot is disabled. Set WHATSAPP_PHONE_NUMBER to enable it.');
  }

  if (CONFIG.TELEGRAM.ENABLED) {
    try { const { startTelegramBot } = require('./src/telegramBot'); telegramBot = await startTelegramBot(CONFIG); } catch (error) { console.error('Telegram bot failed to start. Other services will continue running.'); console.error(error.message); if (process.env.APP_ENV !== 'production') console.error(error.stack); }
  } else {
  ...
  try {
    ...
    console.error('Schedulers/Workers failed to start.');
    console.error(error.message);
    if (process.env.APP_ENV !== 'production') console.error(error.stack);
  }
