...
async function detectAndHandle(chatId, normalizedBody, dbUser, isAdmin) {
  ...
  if (dobIndex !== -1) {
    ...
    let planServices = [];
    if (dbUser) {
      const planId = dbUser.plan_id || dbUser.subscription_plan || 'free';
      try {
        const planRows = await query('SELECT service_id FROM plan_services WHERE plan_id = ?', [planId]);
        planServices = planRows.map((row) => row.service_id);
      } catch (_) {}
    }
    ...
    if (dbUser && dbUser.rate_limit_overrides) {
      try {
        const overrides = typeof dbUser.rate_limit_overrides === 'string'
          ? JSON.parse(dbUser.rate_limit_overrides || '{}')
          : (dbUser.rate_limit_overrides || {});
    ...
    const isPremium = !!(dbUser && (dbUser.plan_id || dbUser.subscription_plan) === 'premium');
