...
async function addAuthorizedEntry(channel, type, id, extras = {}) {
  ...
  if ((c === 'wa' || c === 'whatsapp') && t === 'user') {
    ...
    await repo.updateUserProfile(digits, { name: extras.name || '', plan_id: extras.plan || extras.subscription_plan || extras.plan_id || 'free', expiry_date: extras.expiry_date || '' });
    return u;
  }
  ...
  if ((c === 'tg' || c === 'telegram') && t === 'user') {
    ...
    await repo.updateUserProfile(id, { name: extras.name || '', plan_id: extras.plan || extras.subscription_plan || extras.plan_id || 'free', expiry_date: extras.expiry_date || '' });
    return u;
  }
