...
const { buildTrackedItemsMessage, refreshAllTrackedApplications, removeVahanTrackEverywhere } = require('../services/trackingControlService');
...
apiQueue.process(async (job) => {
 ...
 if (job.command === 'track') { const s = await statusService.getStatusSnapshot(payload.appNo, { keepFile: true }); const p = s.filePath; await sendImageFile(transport, chatId, p); cleanup(p); return { ok: true }; }
 ...
 if (job.command === 'add_track') { await addAutoTrack({ appNo: payload.appNo, transport, chatId, dob: payload.dob || '', tag: payload.tag || '' }); await sendText(transport, chatId, `Tracking added for ${payload.appNo}.`); return { ok: true }; }
 if (job.command === 'add_track_rc') { const r = await addVahanTrack(chatId, payload.appNo, payload.tag || '', transport); await sendText(transport, chatId, r.created ? `Vahan tracking added for ${payload.appNo}.` : `Vahan tracking already exists for ${payload.appNo}.`); return { ok: true }; }
 ...
 if (job.command === 'list_track') { await sendText(transport, chatId, buildTrackedItemsMessage()); return { ok: true }; }
