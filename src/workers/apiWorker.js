...
apiQueue.process(async (job) => {
 ...
 if (job.command === 'track') { const s = await getTrackingSnapshot(payload.appNo, payload.dob || '', { keepFile: true, filename: `Track_${payload.appNo}.jpg` }); const p = s.filePath; await sendImageFile(transport, chatId, p); cleanup(p); return { ok: true }; }
 ...
 if (job.command === 'list_track') { await refreshUserTrackedData(chatId, transport); const p = await imageGeneratorService.generateStatusImage(chatId); await sendImageFile(transport, chatId, p); cleanup(p); return { ok: true }; }
 ...
 if (job.command === 'track_status') { await refreshUserTrackedData(chatId, transport); const p = await imageGeneratorService.generateStatusImage(chatId); await sendImageFile(transport, chatId, p); cleanup(p); return { ok: true }; }
