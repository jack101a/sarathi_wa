...
async function sendWhatsAppText(chatId, text, options = {}) {
  if (!activeWhatsAppClient) throw new Error('WhatsApp client is not ready.');
  return replyQueue.send(() => activeWhatsAppClient.sendMessage(chatId, text, options));
}
...
async function sendWhatsAppMedia(chatId, buffer, mimeType, filename, caption) {
  ...
  return replyQueue.send(() => activeWhatsAppClient.sendMessage(chatId, media, { caption }));
}
